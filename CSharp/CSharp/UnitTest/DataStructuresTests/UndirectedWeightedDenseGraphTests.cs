...
namespace UnitTest.DataStructuresTests
{
    public class UndirectedWeightedDenseGraphTests
    {
        ...
        public void AddEdge_ShouldCreateEdge_WithProvidedWeight()
        {
          ...
          Assert.AreEqual("s", edgeToS.Destination);
          Assert.AreEqual("a", edgeToS.Source);
          Assert.AreEqual(1, edgeToS.Weight);

            var edgeToZ = graph.GetEdge("a", "z");
          Assert.AreEqual("z", edgeToZ.Destination);
          Assert.AreEqual("a", edgeToZ.Source);
          Assert.AreEqual(2, edgeToZ.Weight);
          ...
          Assert.AreEqual(2, graph.IncomingEdges("a").Count());
        }
        ...
        public void UpdateEdgeWeight_ShouldUpdateWeight_OfCSpecificEdge()
        {
            ...
            graph.AddEdge("a", "s", 1);
            graph.AddEdge("a", "z", 2);
          Assert.AreEqual(1, graph.GetEdgeWeight("a", "s"));

            graph.UpdateEdgeWeight("a", "s", 10);
          Assert.AreEqual(10, graph.GetEdgeWeight("a", "s"));
        }
