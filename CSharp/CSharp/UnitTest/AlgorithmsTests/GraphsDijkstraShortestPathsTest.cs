...
namespace UnitTest.AlgorithmsTests
{
    public class GraphsDijkstraShortestPathsTest
    {
        ...
        public void ShortestPathTo_ReturnSingleVertex_WhenDestinationIsSameAsSource()
        {
          ...
          Assert.AreEqual("a", result.Single());
        }
        ...
        public void ShortestPathTo_FindShortestPath_WhenThereIsOnlyOnePath()
        {
          ...
          Assert.AreEqual(3, result.Count());
            Assert.Contains("a", result.ToArray());
            Assert.Contains("c", result.ToArray());
            Assert.Contains("d", result.ToArray());
          Assert.AreEqual(2, dijkstra.DistanceTo("d"));
        }
        ...
        public void ShortestPathTo_FindShortestPath_WhenThereIsPossibleMultiplePaths()
        {
          ...
          Assert.AreEqual(3, result.Count());
            Assert.Contains("a", result.ToArray());
            Assert.Contains("b", result.ToArray());
            Assert.Contains("d", result.ToArray());
          Assert.AreEqual(2, dijkstra.DistanceTo("d"));
        }
        ...
        public void ShortestPathTo_FindShortestPath_WhenEdgeHaveDifferentWeight()
        {
          ...
          Assert.AreEqual(3, shortestToZ.Count());
            Assert.Contains("s", shortestToZ.ToArray());
            Assert.Contains("t", shortestToZ.ToArray());
            Assert.Contains("z", shortestToZ.ToArray());
          Assert.AreEqual(10, dijkstra.DistanceTo("z"));

            var shortestToY = dijkstra.ShortestPathTo("y");
            Assert.NotNull(shortestToY);
          Assert.AreEqual(3, shortestToY.Count());
            Assert.Contains("s", shortestToY.ToArray());
            Assert.Contains("x", shortestToY.ToArray());
            Assert.Contains("y", shortestToY.ToArray());
          Assert.AreEqual(11, dijkstra.DistanceTo("y"));
        }
        ...
        public void DistanceTo_ReturnInfinity_WhenVertexIsNotAchievable()
        {
            ...
            var dijkstra = new DijkstraShortestPaths<DirectedWeightedSparseGraph<string>, string>(graph, "a");
          Assert.AreEqual(long.MaxValue, dijkstra.DistanceTo("c"));
        }
    }
}
